...
- Folder: my_first_simulation (26 Feb 2025)
    - File: my_first_simulation.wbt (26 Feb 2025)
        - Replaced the tutorial arena with a new custom one with a sphere and a side wall, all using the solid node and "practicing" with the DEF-USE mechanism defined in the tutorial 2 (https://cyberbotics.com/doc/guide/tutorial-2-modification-of-the-environment) that we will use in the project to set up the ambient of the warehouse.
    ...
    - File: Proj_controller_my_controller_v1.py (20 Feb 2025)
        - Added a GPS sensor to the robot to get its position in the world. Removed the distance sensor since it's now useless. Added a very basic algorithm that goes to a set target point and then exits the simulation after printing it has reached the target.
